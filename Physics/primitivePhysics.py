...
class Swerve:
  ...
  def updatePositions(self):
    ...
    y = 0
    for wheel in wheels:
      x += cos(self.vectors[wheel][0])*self.vectors[wheel][1]
      y += sin(self.vectors[wheel][0])*self.vectors[wheel][1]
    self.position[0] += x
    self.position[1] += y
  ...
  def swerveWheels(self, direction):
    ...
    self.motor_velocities["frswerve"] = direction
    self.motor_velocities["brswerve"] = direction
    self.motor_velocities["flswerve"] = direction
    self.motor_velocities["blswerve"] = direction
